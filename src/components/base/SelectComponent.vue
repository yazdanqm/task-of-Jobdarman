...
<template>
  <div ref="selectBox" class="relative">
    <div
      ...
      <div class="text-[16px] lg:text-[18px]">{{ selected }}</div>
    ...
    <div :class="{'!opacity-100 !visible !top-[60px]' : showOptions}"
      ...
      <div v-for="(item , index) in options" :key="'option' + index" @click="changeSelectedOption(item)"
           class="rounded-full px-3 py-1 bg-transparent hover:bg-[#ececec] transition-all cursor-pointer text-[16px] lg:text-[18px]">
        {{ item }}
      </div>
    </div>
  </div>
